...
public class Player {
    ...
    public void addAskedCard(Card card, Player p) {
    ...
    public Card askCard (boolean lowest, Player p) { // true = lowest, false = highest
        Card askedCard;
        if (lowest) {
            askedCard = p.hand.get(0);
        } else {
            askedCard = p.hand.get(hand.size() - 1);
        }
        return askedCard;
    }
}
